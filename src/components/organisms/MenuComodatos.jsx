...
function MenuComodatos() {
    ...
    const handlerClickA = () => {
        MySwal.fire({
            ...
            html: <FormularioVehiculos onChange={(values) => (formRef.current = values)} tipoVehiculo="Comodatos"/>,
    ...
    const handlerClickE =  () => {
        MySwal.fire({
            ...
            if(data.TipoVehiculo!="Comodato"){
                throw new Error('El Vehiculo seleccionado no es comodato');
            }
            setForm(data)
            MySwal.fire({
                title: 'Ingresa los datos del Vehiculo',
                html: <FormularioVehiculosEditar onChange={(values) => (formRef.current = values)} tipoVehiculo="Comodatos" data={data}/>,
                showCloseButton: true,
    ...
    <div className="grid w-full grid-cols-2 gap-4 mt-16 bg-slate-200 lg:flex lg:justify-evenly">
